...
def dropout(input_tensor, dropout_keep_rate):
    """Perform dropout.

    Args:
      input_tensor: float Tensor.
      dropout_keep_rate: Python float. The probability of dropping out a value (NOT of
        *keeping* a dimension as in `tf.nn.dropout`).

    Returns:
      A version of `input_tensor` with dropout applied.
    """
    if dropout_keep_rate is None or dropout_keep_rate == 1.0:
        return input_tensor

    output = tf.nn.dropout(input_tensor, dropout_keep_rate)
    return output
