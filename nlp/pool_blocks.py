...
from nlp.nn import masked_reduce_max, masked_reduce_mean, get_var, minus_mask, get_last_output


def SWEM_pool(seqs, mask=None, reduce='CONCAT_MEAN_MAX', scope=None, reuse=None,
    ...
    with tf.variable_scope(scope or 'SWEM_Block', reuse=reuse):
        if reduce == 'LAST':
            ...
            pooled = get_last_output(seqs, length)
        elif reduce == 'FIRST':
            pooled = tf.squeeze(seqs[:, 0:1, :], axis=1)
        elif reduce == 'MAX':
